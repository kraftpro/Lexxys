...
public class TestAttrib
{
	...
	{
		AttribNamePair[] a = [new AttribNamePair("class", new AttributeValue("zero", "one"))];
		a[0].Value.Add("two");
		a[0].Value.Add("three");
		a[0] = new AttribNamePair("class", new AttributeValue("zero"));
		a[0].Value.Add("one");
		a[0].Value.Add("two");
		a[0].Value.Add("three");
	}
...
readonly struct AttribNamePair
{
	public readonly string Name;
	public readonly AttributeValue Value;

	public AttribNamePair(string name, AttributeValue value)
	{
		Name = name ?? throw new ArgumentNullException(nameof(name));
		Value = value;
	}

	public static implicit operator AttribNamePair((string Name, AttributeValue Value) value) => new AttribNamePair(value.Name, value.Value);
}
