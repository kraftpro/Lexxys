...
using HashCode = Lexxys.HashCode;
using Tokenizer = Lexxys.Tokenizer;
using Comparer = Lexxys.Comparer;
...
namespace Lexxys.Test.Con.Core.Xml2
{
	...
	{
		private static readonly Logger Log = new Logger("XmlTools");
		...
		private static bool TryReflection(XmlLiteNode node, Type returnType, out object result)
		{
			...
			SortedSet<string> skipped = new SortedSet<string>(StringComparer.OrdinalIgnoreCase);
			...
			if (missings.Count > 0 || skipped.Count > 0)
			{
				...
				static void GetFieldValue(XmlLiteNode node, string name, Type itemType, IReadOnlyCollection<string> missings, Func<object> getter, Action<object> setter)
				{
					...
					else
					{
						...
						{
							object value = getter();
							TryCollection(XmlLiteNode.Empty, node.Elements.Where(o => String.Equals(o.Name, singular, StringComparison.OrdinalIgnoreCase)), itemType, ref value);
						}
					}
				}
			}
			...
			foreach (var item in missings)
			{
				var element = node.Element(item, StringComparer.OrdinalIgnoreCase);
				if (element.IsEmpty)
					continue;
		...
		private class AttributedConstructor
		{
			...
			public static AttributedConstructor Create(Type type, Dictionary<string, object> arguments)
			{
				...
				for (int i = 0; i < constructors.Length; ++i)
				{
					parameters = parametersSet[i];
					index = new BitArray(parameters.Length);
					for (int j = 0; j < index.Length; ++j)
					{
						string parameterName = parameters[j].Name;
						index[j] = arguments.ContainsKey(parameterName);
						if (!index[j] && !parameters[j].HasDefaultValue)
						{
							index = null;
							break;
						}
				...
				else
				{
					...
					for (int i = 0; i < index.Length; ++i)
					{
						if (!index[i])
						{
							args[i] = Expression.Convert(Expression.Constant(parameters[i].DefaultValue), parameters[i].ParameterType);
						}
						else
						{
