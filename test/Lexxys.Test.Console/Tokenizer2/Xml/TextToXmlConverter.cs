...
namespace Lexxys.Tokenizer2.Xml
{
	...
	public ref struct TextToXmlConverter
	{
		...
		private TextToXmlConverter(in CharStream stream, string sourceName, TextToXmlOptionHandler optionHandler, MacroSubstitution macro = null)
		{
			int tabSize = 4;
			_stream = stream;
			_nodeScanner = new TokenScanner(_stream,
				new WhiteSpaceTokenRule(false, true),
				new CommentsTokenRule(LexicalTokenType.IGNORE, Comments),
				new InlineTextTokenRule(tabSize),
			...
			_nodeScanner.SetFilter(new IndentFilter());
			_nodeScanner.SetFilter(_back);
		...
		private static string SubstituteMacro(string value)
		{
			...
			while (i < value.Length)
			{
				...
				if (j < 0)
					break;
				var macro = value.Substring(i + 3, j - i - 3);
				if (!String.IsNullOrWhiteSpace(macro))
				{
					string subst = Config.GetValue<string>(macro);
					if (subst != null)
		...
		private class InlineTextTokenRule: LexicalTokenRule
		{
			...
			public InlineTextTokenRule(int tabSize = default)
				: this(TEXT, tabSize)
			{
			}

			private InlineTextTokenRule(LexicalTokenType textTokenType, int tabSize = default)
			{
				TokenType = textTokenType;
				_comments = new CppCommentsTokenRule();
				_tabSize = tabSize <= 0 ? 4 : tabSize;
			}
			...
			public override LexicalToken TryParse(CharStream stream)
			{
				...
				if (i < 0)
					return null;
				stream.Forward(i);
				string text = lines.Count == 0 ? String.Empty: Strings.CutIndents(lines.ToArray(), _tabSize);
		...
		private void PopNode()
		{
			...
			_currentNodePath.Length = _currentNodePath.Length - (node.Name.Length + 1);
		}
