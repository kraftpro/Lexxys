...
namespace Lexxys.Tokenizer2
{
	...
	public sealed class LexicalToken
	{
		...
		public LexicalToken(LexicalTokenType tokenType, string text, int position, CultureInfo cultureInfo = default)
		{
		...
		/// <param name="cultureInfo">The culture of the token</param>
		public LexicalToken(LexicalTokenType tokenType, string text, int position, object value, CultureInfo cultureInfo = default)
		{
			Text = text ?? throw new ArgumentNullException(nameof(text));
			TokenType = tokenType;
			Position = position;
