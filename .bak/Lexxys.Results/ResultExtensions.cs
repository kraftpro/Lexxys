...
public static class ResultExtensions
{
	...
	/// <param name="func">A function that takes the error from the failed result and returns an alternative result of the same type. Cannot
	/// be null.</param>
	/// <returns>The original result if it is successful; otherwise, the result returned by the specified function.</returns>
	...
	/// <param name="func">A function to invoke if the result is an error. The function receives the error information and returns a task that
	/// produces a new result.</param>
	/// <returns>A task that represents the asynchronous operation. The task result is either the original result if it is
	/// successful, or the result returned by the specified function if the original result is an error.</returns>
	...
	/// <param name="func">A function to invoke if the original result is an error. The function receives the error result and returns a task
	...
	/// <param name="func">A function to invoke if the awaited result is an error. The function receives the error result and returns an
	...
	/// action is only called if the result indicates failure; otherwise, it is ignored.</remarks>
	/// <typeparam name="T">The type of the value contained in the result.</typeparam>
	...
	public static void OnError<T>(this Result<T> result, Action<ErrorResult> action)
	{
		if (result.IsFailure)
			action(result.Error);
	}
	...
	/// operations. The action is only invoked if the result indicates failure; otherwise, it is ignored.</remarks>
	/// <typeparam name="T">The type of the value contained in the result.</typeparam>
	/// <param name="result">A task that produces a result of type <see cref="Result{T}"/> to be checked for failure.</param>
	...
	public static async Task OnError<T>(this Task<Result<T>> result, Func<ErrorResult, Task> action)
	{
		var r = await result;
		if (r.IsFailure)
	...
	/// <summary>
	/// Invokes the specified action if the asynchronous result represents a failure.
	/// </summary>
	/// <remarks>Use this method to handle errors in asynchronous result operations without interrupting the normal
	/// flow. The action is only called if the result indicates failure; otherwise, it is not invoked.</remarks>
	/// <typeparam name="T">The type of the value contained in the result.</typeparam>
	/// <param name="result">A task that produces a result of type <see cref="Result{T}"/> to be checked for failure.</param>
	/// <param name="action">The action to execute if the result is a failure. Receives the error information as an argument.</param>
	...
	/// failure.</returns>
	public static async Task OnError<T>(this Task<Result<T>> result, Action<ErrorResult> action)
	{
		var r = await result;
		if (r.IsFailure)
			action(r.Error);
	...
	public static Result<T> Assert<T>(this Result<T> result, Func<T, bool> predicate, Func<T, ErrorResult> error) =>
		!result.IsSuccess ? result:
		predicate(result.Value) ? result: error(result.Value);
	...
	/// <returns>A new result containing the original value if validation succeeds, or the error result returned by the error function if validation fails.
	/// If the original result is not successful, it is returned unchanged.</returns>
	public static Result<T> Assert<T>(this Result<T> result, Func<T, bool> predicate, string message, string? title = null, int statusCode = 0, params (string Key, object? Value)[]? data) =>
		!result.IsSuccess ? result:
	...
	/// <returns>A new result containing the original value if validation succeeds, or the error result returned by the error function if validation fails.
	/// If the original result is not successful, it is returned unchanged.</returns>
	public static async Task<Result<T>> Assert<T>(this Task<Result<T>> result, Func<T, bool> predicate, Func<T, ErrorResult> error)
	{
		var r = await result;
		return !r.IsSuccess ? r: predicate(r.Value) ? r: error(r.Value);
	}
	...
	/// <returns>A new result containing the original value if validation succeeds, or the error result returned by the error function if validation fails.
	/// If the original result is not successful, it is returned unchanged.</returns>
	public static async Task<Result<T>> Assert<T>(this Task<Result<T>> result, Func<T, bool> predicate, string message, string? title = null, int statusCode = 0, params (string Key, object? Value)[]? data)
	{
		var r = await result;
...
/*
	...
	public static Result<T> Assert<T>(FailureResult<T>? result, bool condition, string item, string message)
		=> condition ? result: result ??= new FailureResult<T>(message);

	public static Result<int> Play(int value)
	{
		Result<int>
			.Assert(value > 0, "Value must be greater than zero")
			.Assert(value < 100, "Value must be less than one hundred")
			.Then(value)
			.Otherwise(error => Result.Fail<int>("Default value due to error: " + error.Message));
	}
*/
}
