...
public static class FieldValidator
{
	...
	public static ValidationResults FieldValue<T>(T? value, T min, T max, string field)
		where T: struct, IComparable<T>
	{
		return FieldValue(value, (T?)min, max, field, true);
	}

	public static ValidationResults FieldValue<T>(T? value, T min, T max, string field, bool nullable)
		where T: struct, IComparable<T>
	{
		return FieldValue(value, (T?)min, max, field, nullable);
	}
	...
	{
		return Format(value, ErrorDataType.UsState, _ => Check.UsStateCode(value, true), 0, field, nullable);
	}

	public static ValidationResults CountryCode(string? value, string field, bool nullable = false)
	{
		return Format(value, ErrorDataType.Country, _ => Check.CountryCode(value, true), 0, field, nullable);
	}
