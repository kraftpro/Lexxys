...
namespace Lexxys.Data
{
	public static class Check
	{
		...
		private static readonly LocalCache<(string Table, string Key, int Value), bool> __localCache = new LocalCache<(string, string, int), bool>(
			capacity: Config.Default.GetValueInRange(ConfigReference + ":cacheCapacity", 16, 128 * 1024, 8 * 1024),
			timeToLive: Config.Default.GetValueInRange(ConfigReference + ":cacheTimeout", new TimeSpan(0, 0, 10), new TimeSpan(1, 0, 0), new TimeSpan(0, 10, 0)));
		...
		private static readonly IValue<string> UsStateCodes = Config.Default.GetValue("Lexxys.Check.UsStateCodes", "AA AK AL AP AR AS AZ CA CO CT DC DE FL FM GA GU HI IA ID IL IN KS KY LA MA MD ME MI MN MO MP MS MT NC ND NE NH NJ NM NV NY OH OK OR PA PR PW RI SC SD TN TX UT VA VI VT WA WI WV WY");
