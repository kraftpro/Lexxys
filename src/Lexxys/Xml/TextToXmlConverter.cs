...
namespace Lexxys.Xml
{
	...
	public ref struct TextToXmlConverter
	{
		...
		public static XmlLiteNode ConvertToXmlLite(Node node, bool ignoreCase)
		{
			...
			if (node.HasAttribs)
			{
				attrib = node.Attribs!.ToArray();
				for (int i = 0; i < attrib.Length; ++i)
				{
					attrib[i] = KeyValue.Create(attrib[i].Key, SubstituteMacro(attrib[i].Value));
				}
			}
			return new XmlLiteNode(node.Name, SubstituteMacro(node.GetValue()), ignoreCase || node.IgnoreCase ? StringComparer.OrdinalIgnoreCase : StringComparer.Ordinal, attrib, child);
		}
		...
		private Node? ScanNode(ref CharStream stream)
		{
			...
			if (token.TokenType != LexicalTokenType.INDENT)
			{
				...
				{
					node.AppendNlValue(token.GetString(stream));
				}
				else
				{
					Back();
		...
		public class Node
		{
			...
			public List<KeyValuePair<string, string>>? Attribs => _attrib;
			...
			public bool HasAttribs => _attrib?.Count > 0;
			
			public bool HadChildren => _child?.Count > 0;

			public List<Node>? GetChildren() => _child;
