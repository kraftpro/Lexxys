...
namespace Lexxys.Xml
{
	public static partial class XmlTools
	{
		...
		private class CollectionType
		{
			...
			public CollectionType(Type entity, Type collection, Type item)
			{
				Entity = entity;
			...
			if (type.IsGenericType)
			{
				...
				else if (tt.Length == 2)
				{
					...
					if (type.IsAssignableFrom(dictionaryType))
					{
						...
						return new CollectionType(dictionaryType, typeof(ICollection<>).MakeGenericType(itemType), itemType);
					}
				}
			}
