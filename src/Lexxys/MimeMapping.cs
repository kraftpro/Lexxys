...
{
	private const string ConfigSection = "lexxys.mime";
	...
	private static IReadOnlyDictionary<string, string> Mappings
	{
		...
		{
			if (_mappings == null)
				Interlocked.CompareExchange(ref _mappings, BuildMappings(), null);
			return _mappings;
		}
	}
	...
	public static string? GetExtension(string mimeType, string? defaultExtension = null)
	{
		...
		if (mimeType.IndexOf('/') < 0)
			return "." + mimeType.ToLowerInvariant();
		return Mappings.TryGetValue(mimeType, out var extension) ? extension: defaultExtension;
	}
}
