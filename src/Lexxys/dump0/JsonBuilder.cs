...
/// </summary>
[Obsolete]
public abstract class JsonBuilder: IJsonBuilder
{
	/// <summary>
			...
			switch (cn.GetTypeCode())
			{
				...
				case TypeCode.UInt64:
					return value is Enum ?
						"\"" + Strings.ToNamingRule(cn.ToString(CultureInfo.InvariantCulture), NamingRule) + "\"":
						cn.ToString(CultureInfo.InvariantCulture);
				case TypeCode.Single:
					var f = (float)value;
					return Single.IsNaN(f) || Single.IsInfinity(f) ? NullValue: f.ToString(CultureInfo.InvariantCulture);
				case TypeCode.Double:
					var d = (double)value;
					return Double.IsNaN(d) || Double.IsInfinity(d) ? NullValue: d.ToString(CultureInfo.InvariantCulture);
...
{
	private static new readonly byte[] NullValue = "null"u8.ToArray();
