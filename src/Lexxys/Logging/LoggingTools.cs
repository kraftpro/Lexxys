...
public static class LoggingTools
{
	...
	public static LogType ToLogType(LogLevel logLevel) => (uint)logLevel >= (uint)LogLevel.Critical ? LogType.Output: (LogType)(LogLevel.Critical - logLevel);

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static LogLevel ToLogLevel(LogType logType) => (LogLevel)((uint)logType >= (uint)LogType.MaxValue ? 0: LogType.MaxValue - logType);
