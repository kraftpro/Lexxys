...
public static class Crypto
{
	...
	public static Hasher Hasher(string algorithm) => _hasherCache.GetOrAdd(
		...
		o => new Hasher((IHasherAlgorithm)CreateInstance(CryptoProviderType.Hasher, o.Algorithm)));

	public static Hasher Hasher(string algorithm, object key) => _hasherCache.GetOrAdd(
		new AlgKeyPair(algorithm, key),
		o => new Hasher((IHasherAlgorithm)CreateInstance(CryptoProviderType.Hasher, o.Algorithm, o.Key)));

	public static Encryptor Encryptor(string algorithm, object key) => _encryptorCache.GetOrAdd(
		new AlgKeyPair(algorithm, key),
		o => new Encryptor((IEncryptorAlgorithm)CreateInstance(CryptoProviderType.Encryptor, o.Algorithm, o.Key)));

	public static Decryptor Decryptor(string algorithm, object key) => _decryptorCache.GetOrAdd(
		new AlgKeyPair(algorithm, key),
		o => new Decryptor((IDecryptorAlgorithm)CreateInstance(CryptoProviderType.Decryptor, o.Algorithm, o.Key)));
	...
	private static object CreateInstance(CryptoProviderType providerType, string name, params object?[] args)
	{
		...
		try
		{
			return (args is not { Length: >0 } ? Factory.TryConstruct(item.Type, true): Factory.TryConstruct(item.Type, true, args)) ?? throw new ArgumentException(SR.Factory_CannotFindConstructor(item.Type, args?.Length ?? 0));
		}
