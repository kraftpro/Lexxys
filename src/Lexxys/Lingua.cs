...
public static class Lingua
{
	...
	private static class PluralRules
	{
		...
		private struct PluralRule
		{
			...
			public readonly string SingularEnding;
		...
		private static Rule PrepareRule()
		{
			...
			for (int i = 0; i < rr.Length; ++i)
			{
				...
				if (k < ww.Length)
				{
					if (rr[i].Classical)
					{
						ir.Spc.Ending.Add(new Ending(rr[i].SingularEnding.Length, rr[i].PluralEnding));
						Append(rspc, ww, rr[i].SingularEnding);
						ir.Psc.Ending.Add(new Ending(rr[i].PluralEnding.Length, rr[i].SingularEnding));
						Append(rpsc, ww, rr[i].PluralEnding);
					}
					else
					{
						ir.Spu.Ending.Add(new Ending(rr[i].SingularEnding.Length, rr[i].PluralEnding));
						Append(rspu, ww, rr[i].SingularEnding);
						if (rr[i].SingularEnding != "is")
						{
							ir.Psu.Ending.Add(new Ending(rr[i].PluralEnding.Length, rr[i].SingularEnding));
