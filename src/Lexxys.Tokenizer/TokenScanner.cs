...
public struct TokenScanner
{
	...
	private readonly int[] _asciiRules;
	private readonly int[] _extraRules;
	...
	/// <summary>
	/// Initializes a new <see cref="TokenScanner"/> with the list of the <see cref="LexicalTokenRule"/>s and the collection of the <see cref="ITokenFilter"/>s.
	/// </summary>
	/// <param name="filters">Collection of the <see cref="ITokenFilter"/>s.</param>
	/// <param name="rules">Collection of the <see cref="LexicalTokenRule"/>s.</param>
	...
	/// <summary>
	/// Gets the collection of lexical token rules and their enabled states.
	/// </summary>
	public readonly IReadOnlyList<(bool Enabled, LexicalTokenRule Rule)> Rules => _rules;

	/// <summary>
	/// Enables or disables rules matching the specified predicate.
	/// </summary>
	/// <param name="predicate">A function that determines which rules to enable or disable.</param>
	/// <param name="enable"><see langword="true"/> to enable matching rules; <see langword="false"/> to disable them.</param>
	/// <returns><see langword="true"/> if any rule's enabled state was changed; otherwise, <see langword="false"/>.</returns>
	...
	/// <summary>
	/// Builds lookup tables for efficient rule matching by mapping ASCII characters to rule indices and collecting rules
	/// that require extended processing.
	/// </summary>
	private readonly void ApplyRules()
	{
