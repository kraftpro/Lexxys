...
namespace Lexxys;
internal class SmbFileNameFormater
{
}
...
/// </summary>
public class SmbFileNameFormatter: IBlobNameFormatter
{
	...
	/// </summary>
	public static readonly SmbFileNameFormatter Default = new SmbFileNameFormatter();
	...
	/// <summary>
	/// Creates a new <see cref="SmbFileNameFormatter"/>
	/// </summary>
	...
	public SmbFileNameFormatter(int directoryCount = default, int fileCount = default, int radix = default, bool flat = default, char pathSeparator = default, string? temporaryFolder = default, DirectoryGenerationMode mode = default)
	{
		if (directoryCount < 0)
	...
	{
		if (id is null)
			return MakePath(null, InitSalt(), type ?? string.Empty);
		if (id is long index)
			return MakePath(index, InitSalt(), type ?? string.Empty);
		if (id is ulong uindex)
			return MakePath((long)uindex, InitSalt(), type ?? string.Empty);
		if (id is string strId && long.TryParse(strId, out var parsedIndex))
			return MakePath(parsedIndex, InitSalt(), type ?? string.Empty);
		throw new ArgumentException("Invalid id type. Expected long or ulong.", nameof(id));
	}
